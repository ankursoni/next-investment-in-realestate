...
from core.config import Config, DataRepositoryType, DataSourceType
...
def parse_cli_args_with_defaults() -> CLIArgs:
    ...
    parser.add_argument(
        ...
        help="Data repository type: realestate (default) or domain",
    )
    parser.add_argument(
        "--data-source-type", help="Data source type: sqlite (default) or mongodb"
    )
    parser.add_argument(
        "--run-sql-migrations", help="Run sql migrations: true (default)"
    )
    ...
    cli_args = {
        ...
        "run_sql_migrations": (
            json.loads(args.run_sql_migrations) if args.run_sql_migrations else None
        ),
    }
    result = CLIArgs(
...
def log_config_settings(conf: Config) -> None:
